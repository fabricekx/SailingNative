...
const ConfigTiller = () => {
    ...
    const myPilotConfig = useRef<ActuatorControllerTest | null>(null);
  ...
  const BabordTribord = () => {
    const changeSens = (selectedSens: string) => {
      setSens(selectedSens === "normal"); // true si normal
      appConfig.sens = sens;
...
console.log(appConfig);
    return (
      <View>
        ...
        <Text>
          {" "}
          Appuyer sur les fleches gauche et droite pour vérifier que la barre
          commande correctement le bateau
        </Text>
        <ClickButton
  ...
  return (
    <View>
      ...
      <Pressable
        className=" w-20 min-h-10 items-center bg-white dark:bg-slate-600 rounded-lg p-2 m-2"
